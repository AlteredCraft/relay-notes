  ...
  *Done when: with "On-device (Whisper)" selected in Settings, tap → record → stop produces a non-empty transcript on the iPhone 15 Pro Max, and "Apple Speech" still works unchanged.*
  ...
  - [~] **T1.2 — Whisper wired into the recorder flow.** Sliced into six substages (T1.2a–T1.2f) on 2026-06-10 so each is independently shippable. T1.2a + T1.2b shipped 2026-06-10/11; T1.2c–f remain. The model bundle moved off "hand-converted + bundled" toward "downloaded from HuggingFace's `mlx-community/whisper-small.en-fp16` at a pinned commit + cached in Application Support" — substages add the abstraction (T1.2a), the store (T1.2b), the cached actor that consumes the store (T1.2c), the streaming session that the recorder feeds (T1.2d), the Settings UI for download/delete (T1.2e), and the recorder-side placeholder UX + missing-model guard (T1.2f).
    *Done when: pick Whisper → download → record → stop → transcript saved to a `Note`, Apple Speech still selectable and working.*
    ...
    - [~] **T1.2c — Cached `WhisperMLXTranscriber` (actor + store wiring).** Code shipped 2026-06-11; device validation pending. `WhisperMLXTranscriber` converted `nonisolated final class` → `actor`: single-entry `LoadedAssets` cache (`WhisperModel` + `WhisperTokenizer` + `mel_filters`, keyed by `WhisperModelLocation`; single-entry by design — two resident `small.en` weight sets would be ~1 GB fp16). `WhisperModelStore` injected (optional): new `store.activeLocation` (`status == .ready ? location : nil`) is read *per transcribe call* with fallback `.bundled`, so a download/delete mid-session takes effect on the next call without latching. `TranscriberFactory` gained a `whisperModelStore:` init param (default `nil`; ContentView constructs/passes the store in T1.2e per the slicing). New `WhisperAudio.logMelSpectrogram(audio:filters:padding:)` overload consumes the cached filterbank. **Isolation gotcha worth remembering:** `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor` infers `@MainActor` onto an actor's *synchronous init* (methods keep actor isolation — verified via compile-error probe), and both opt-out spellings (`nonisolated init`, `nonisolated actor`) are rejected by the Xcode 26.5 toolchain — accepted the main-actor init (every construction site is `@MainActor` anyway) and documented it on the type. Tests: 5 new simulator-safe (`WhisperMLXTranscriberTests` — fallback/ready/mid-session resolution + factory injection) + 1 device-gated cache-identity probe + 2 store `activeLocation` tests; `scripts/add_test_file.rb` generalizes the pbxproj test-wiring script. 41 tests / 8 suites green. *(2026-06-11)*
      *Done when: warm transcribe time on `ls_test.flac` drops to roughly the encoder + decoder cost alone (no model reload); cold-then-warm behavior visible in `MLXSmoke` output.* — **pending device run**: `MLXSmoke.runWhisperTranscribe` now does a cold-then-cached double pass and prints both timings.
    - [ ] **T1.2d — Streaming session for Whisper (PCM accumulate → finalize-only decode).** Implement `WhisperMLXTranscriber.makeStreamingSession(options:)`. New `WhisperStreamingSession` buffers `AVAudioPCMBuffer`s in memory during `feed(_:)`, emits **zero** updates on the `updates: AsyncStream<String>`, and on `finish()` concatenates PCM → calls the cached transcribe path → returns the single final transcript. `cancel()` drops the buffer.
      *Done when: `RecorderViewModel.stopAndTranscribe` succeeds against `WhisperMLXTranscriber` (manual test via the existing recorder flow with Whisper engine selected — UI placeholder ships in T1.2f).*
    - [ ] **T1.2e — Settings activates the Whisper engine row + model download/delete UI.** Replace the "Coming next" placeholder row with a real selectable Whisper engine row. Add a model-management section observing `WhisperModelStore.status` — pre-download button when `.missing`, progress bar + cancel when `.downloading`, "Installed" + delete when `.ready`, retry + actionable error when `.failed`. Recording with Whisper requires `.ready` (T1.2f wires the guard); Settings is where the user actually triggers the download. `ContentView` constructs the store and passes it down via env or DI.
    ...
    - [ ] **T1.2f — Recorder placeholder UX + blocked-record guard.** While Whisper is the selected engine: `RecorderView` replaces the live partial transcript card with a placeholder ("Transcript will appear when you stop recording.") + elapsed-time label + audio level meter; `.finalizing` shows the existing "Transcribing…" spinner (no progress %). `RecorderViewModel.startRecording` preflights against `WhisperModelStore.status` — if Whisper is selected but status isn't `.ready`, fail immediately into `.failed(message:)` with an actionable string ("Download the Whisper model from Settings to record with on-device.").
      *Done when: with Whisper selected and the model downloaded, tap → record → stop → transcript saved to a `Note`; with Whisper selected and no model, recording is blocked with a clear message; Apple Speech path unchanged.*
