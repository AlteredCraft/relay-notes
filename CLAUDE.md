...
### Whisper weights are download-only; only small assets are bundled (flat at the .app root)

As of 2026-06-11 the **weights are no longer bundled** — they download at runtime into `Application Support/whisper/small.en/` via `WhisperModelStore` and are the only path to a complete model (`ModelLocation.directory`, renamed from `WhisperModelLocation` in T2.5 — now engine-neutral, shared with Parakeet). The 481 MB `weights.safetensors` still lives on disk in `Relay Notes/Relay Notes/Resources/whisper-small.en/` (gitignored, fetched via `scripts/fetch-whisper-model.sh [tiny.en|small.en]`, which calls `scripts/convert-whisper-assets.py` to convert npz → safetensors since `mlx-swift`'s `loadArrays(url:)` only reads safetensors), but it is **excluded from Copy Bundle Resources** by a `PBXFileSystemSynchronizedBuildFileExceptionSet` membershipExceptions entry (`Relay Notes/Resources/whisper-small.en/weights.safetensors`, alongside `Info.plist`). Dropping it took the built `.app` from ~555 MB to ~74 MB. The **small** staged assets (`config.json`, `gpt2.tiktoken`, `mel_filters.safetensors`) and the `ls_test.flac` fixture are still bundled and picked up automatically by the synchronized-group rule — `WhisperModelStore.stageBundledAssets()` copies the three small files from `Bundle.main` into the model directory at download time; `.bundled` still resolves them (and the fixture), just not the weights. The default variant is `small.en` (2026-06-10 validation: 481 MB FP16 loads on the iPhone 15 Pro Max without the `increased-memory-limit` entitlement). **When changing what's bundled:** add/remove the matching `membershipExceptions` entry and verify the built `.app` contents (`find "<app>" -name '*.safetensors'`).
...
The scheme `Relay Notes` is **shared** (`xcshareddata/xcschemes/`) — that's what makes the CLI test action reproducible; keep it shared. As of 2026-06-11 the suite is 61 tests / 9 suites, ~12 s warm. Remember the gating convention above: MLX-touching tests compile in but never execute on the simulator. UI tests (XCUIAutomation) are still unplanned. New test *files* must be wired into the pbxproj: `ruby scripts/add_test_file.rb <FileName.swift>` (the test target is a plain `PBXGroup`, not a synchronized one).
